...
public class BusinessProfileDao extends HibernateDao implements Dao<BusinessProfile, Long> {
    ...
    @Override
    public void save(BusinessProfile business) {
        Preconditions.checkNotNull(business);
        getCurrentSession().save(business);
    }

    @Override
    public void update(BusinessProfile business) {
        Preconditions.checkNotNull(business);
        getCurrentSession().update(business);
    }

    @Override
    public void delete(BusinessProfile business) {
        Preconditions.checkNotNull(business);
        getCurrentSession().delete(business);
    }
}
